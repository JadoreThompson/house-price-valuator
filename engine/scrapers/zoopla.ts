...
class ZooplaScraper {
  ...
  public async run(): Promise<void> {
    ...
    if (fs.existsSync(cachedUrlPath)) {
      const cachedData = JSON.parse(fs.readFileSync(cachedUrlPath, "utf-8"));
      url = cachedData.url;
    }

    if (url! === undefined || url === "https://www.zoopla.co.uk/500/") {
      url = this.url;
    }
    ...
    while (pageNum <= 40) {
      ...
      try {
        ...
        fs.writeFileSync(
          cachedUrlPath,
          JSON.stringify({ url: currentPage.url() })
        );

        pageNum += 1;
      } catch (error) {
        console.error(
          "Error handling page switch",
          error,
